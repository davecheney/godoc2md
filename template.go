...
| Name | Synopsis |
| ---- | -------- |
{{if not (or (eq $.Dirname "/src") (eq $.Dirname "/src/cmd") $.DirFlat)}}| [..](..) | |{{end}}
{{range .List}}{{if $.DirFlat}}
{{if .HasPkg}}| [{{html .Path}}]({{html .Path}}/{{modeQueryString $.Mode | html}}) | {{html .Synopsis}} |{{end}}
{{else}}| [{{html .Path}}]({{html .Path}}/{{modeQueryString $.Mode | html}}) | {{html .Synopsis}} |{{end}}
{{end}}
{{end}}
- - -
